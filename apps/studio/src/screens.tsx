...
/**
 * Pantallas del Studio (dominio Studio F2, shell B).
 *
 ...
 * `src/data/**` (queryOptions + Zod en la frontera); INV-1 intacto (la red
 * vive solo en gatewayClient).
 *
 * **Por qué este archivo existe aparte de `App.tsx`:** `App.tsx` hacía dos
 * trabajos —ser la RAÍZ (providers + RouterProvider) y ser el MÓDULO DE
 * PANTALLAS que el router monta— y eso cerraba un ciclo real
 * `App → router → App`, que `depcruise` reportaba como error. Separar los dos
 * trabajos rompe el ciclo por donde estaba: la raíz depende del router, el
 * router depende de las pantallas, y las pantallas no dependen de ninguno de
 * los dos.
 */
...
export function RunDetailScreen({
  ...
  // Merge F1+F2: certificado solo con run completado (F1, mata el 409 ruidoso);
  // files y ablation alimentan las lentes (F2, O7/#173.2 y #177/CP6).
