...
class TestGetStepEvidence:
    def test_step_con_capability_job_trae_los_digests(self) -> None:
        ...
        # [C4/M4] El paso trae las constancias de sus DOS verificadores; el de
        # ejecución aporta además una por isla (`step_id = island-{k}`), así
        # que se afirma la cobertura y el desglose, no un conteo que cambia
        # con la granularidad.
        ...
        por_isla = [
            ...
            if a["verifier_id"] == "verifier:pandapower-islanding"
            and a["step_id"] is not None
        ]
        assert por_isla == ["island-0", "island-1"]
