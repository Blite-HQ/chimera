...
fija el caso feliz), inmutabilidad/`extra=forbid`, los `field_validator` de
`DerivationProvenance` (rechaza recetas/inputs/assertions incompletos),
`parse_provenance` (despacho por `kind`) y determinismo del digest bajo la
única puerta de canonicalización (`blite.certificate.canonical.canonicalize`).
"""
...
class TestDerivationProvenanceValidation:
    def test_rejects_recipe_missing_required_keys(self) -> None:
        # Arrange / Act / Assert — recipe sin 'code_ref'
    ...
    def test_rejects_assertion_entry_missing_passed(self) -> None:
        with pytest.raises(ValidationError, match="passed"):
            DerivationProvenance(
                ...
                recipe=_VALID_RECIPE,
                run_id="r1",
                assertions=({"name": "geojson_schema_valid"},),
            )
