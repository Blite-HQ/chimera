...
def _request(backend_id: str = "anthropic/claude", prompt_digest: str = "p" * 64) -> ModelRequest:
...
def test_record_then_replay_round_trip_same_request_finds_the_fixture() -> None:
    ...
    replayer = ModelServer(mode="replay", content_store=store, ctx=_CTX, manifest=manifest)
...
def test_replay_miss_for_an_unseen_backend_even_if_another_backend_was_recorded() -> None:
    store = InMemoryContentStore()
    manifest = InMemoryReplayManifest()
    prompt_digest = "d" * 64
    ...
    replayer = ModelServer(mode="replay", content_store=store, ctx=_CTX, manifest=manifest)
    with pytest.raises(ReplayMissError):
        replayer.call(_request(backend_id="ollama_chat/gpt-oss", prompt_digest=prompt_digest))
