...
- **Hoy (env):** implementación trivial — llave semilla de 32 bytes por `purpose` en env var (`CHIMERA_TRUST_CERT_KEY`, `CHIMERA_JWT_KEY`), `keyid` fijo `:v1`; `rotate()` no implementado (placeholder que levanta `NotImplementedError` documentado, no una promesa vacía).
...
4. **Frontera con Geovanni (§13, no decidido por mí):** dónde vive/se opera la infraestructura de secretos (¿un OpenBao compartido del proyecto o uno por servicio?) es su carril; este puerto solo define cómo el engine LE PIDE una firma — coordinación pendiente, no bloqueante para el freeze.
