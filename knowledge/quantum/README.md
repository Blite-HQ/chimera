...
| [02](02-recetario-formulacion-por-reto.md) | Recetario proponente por reto: grafo→QUBO→Ising→circuito QAOA (G6 cerrado), pipeline kernel Reto 2, VQE/UCCSD/anclas Reto 3                                                                                                                                 | produce los claims que consumen `ExactSolverVerifier` (trust/10) y `ExecutionVerifier` (trust/12); λ/features/seeds → `knowledge/` versionado + `evidence` |
...
> Las notas 05–06 son investigación de consolidación (Dylan, 2026-07-14) que cierra los huecos
> del plan; **pendientes de validación y ratificación de Sebas**, igual que el corpus de
> `knowledge/islanding/`.
