...
| §1.1 objetivo Max-Cut (maximizar corte)                                     | Eq. 11 minimiza el flujo cortado (min-cut ponderado por física)                                                      | ⚠️ **Signo opuesto del objetivo**: misma maquinaria QUBO/Ising, dirección contraria. Decisión de formulación que Sebas debe ratificar contra el enunciado del reto (islanding real = minimizar disrupción) |
...
- Si se porta M.3/M.4, la evidencia del claim del Reto 1 gana campos aditivos naturales: `repair.method` (id M.1–M.5), `repair.flips` (número de flips aplicados), `repair.pre_value` / `repair.post_value` (corte antes/después de reparar) y `connectivity_violations` (C(z) del bitstring crudo). **PENDIENTE: registrarlos en `docs/contract-freeze.md` si se adopta la decisión** — misma advertencia de "contratos fantasma" del README (pendiente №3).
...
- **Punto a ratificar por Sebas:** la dirección del objetivo (Max-Cut del Reto 1 vs min-cut físico de REGRID, §1.8) y la adopción de "Ruta A+" (λ_C en el lazo) como extensión del recetario de la nota 02. **Ratificación de Sebas: PENDIENTE.**
