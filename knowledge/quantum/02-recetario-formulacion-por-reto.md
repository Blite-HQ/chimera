...
2. **Cota de suficiencia:** violar el balance en δ cuesta λδ²; la ganancia máxima posible de corte es ≤ W (peso total). Para que ninguna violación > τ̂ sea rentable: **λ > W / τ̂²**. Con el grid de ejemplo del doc CHIMERA (W = 5.9, τ̂ = 0.3): λ > 65.6. _(⚠️ SUPERSEDIDO [S-F 2026-07-20]: W = 5.9 / τ̂ = 0.3 / λ > 65.6 son del grafo SINTÉTICO de 8 nodos — cr8 se construye desde los datos abiertos del ICE y λ se recalcula sobre la red real; la FÓRMULA de la cota sigue vigente — islanding/01 §1.8.)_
3. **Cota de cordura:** λ demasiado grande aplana el paisaje (el espectro lo domina la penalización y QAOA deja de "ver" el corte). Guía práctica del tutorial de QUBO de Glover et al. (arXiv:1811.11538): penalización del orden de 0.75–1.5× la magnitud estimada del objetivo, y ajuste empírico: resolver → correr el constraint checker → si infactible, λ×2; si factible con margen, probar λ/2.
   - **Cross-reference (bootcamp QWorld OQI, Day 8 — QUBO):** para una restricción penalizada de forma aislada (no el caso de balance de arriba, que ya tiene su propia cota en el punto 2), la sesión enseña la misma familia de cota de suficiencia en su forma más simple: P > peso máximo de una sola arista del grafo, para que violar la restricción nunca sea rentable frente a cualquier ganancia de corte individual. Es un caso particular de la lógica del punto 2, útil como chequeo rápido antes de afinar con Glover et al.
...
- **Impacto en contrato:** produce los claims que consumen `ExactSolverVerifier` (trust/10) y `ExecutionVerifier` (trust/12); λ/features/scaler/seeds viven en `knowledge/` versionado con digest y se registran en `evidence` (§4). **SUPERSEDIDO (S-E 2026-07-18):** los valores de referencia H₂ de §3.5 eran del Reto 3 recetado como VQE/química; el C3 oficial es TFIM/Trotter — su ancla es la **diagonalización exacta** (SciPy/PySCF) computada en el kit C3 (cadenas N ∈ {6,8,12}, criterio oficial ≤5% en N=8), no vectores H₂. No se regeneran.
- **Reconciliación contra la base lógica:** revisada en la consolidación — respeta la separación proponente/verificador (trust/10–12) y ADR-029 (λ es dato versionado en `knowledge/islanding/`, no código del engine). **Decidido (S-E 2026-07-18) — ratificación final de Sebas, ajustable bajo su criterio.**
