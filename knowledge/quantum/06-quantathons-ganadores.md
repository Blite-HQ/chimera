...
| **jetix** (1º Química + top-3 Braket)     | Molecular Energy Landscapes (VQE/HEA para carbon capture)                                        | patrón de química/VQE con narrativa de impacto (ya no es el Reto 3 — C3 oficial: TFIM/Trotter)                     | **MIT** (verificado en vivo)        |
...
- **Decidido (S-E 2026-07-18) — ratificación final de Sebas, ajustable bajo su criterio.** El formato de entrega quedó fijado por el enunciado oficial + P0-6 (paquete reproducible `challenge1/` con `reproduce.py` único), que subsume el formato tipo ETH aquí propuesto; la regla de licencias de §3 queda adoptada (se ejecuta en el PR único de deps de S-G).
