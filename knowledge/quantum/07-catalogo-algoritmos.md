...
| **Goemans–Williamson SDP**                              | relajación semidefinida + redondeo aleatorio; **garantía 0.878** del óptimo        | cuando se necesita cota de calidad SIN solver exacto (n grande); NO este mes: exige solver SDP nuevo (§3) y en n≤30 CP-SAT da el óptimo exacto | N/A (clásico, JACM 1995); el número 0.878 ya calibra el pitch (nota 01 §7)                             | **catalogado**                                                                 | FORMAL_EXACT donde el corpus alcanza; su propia cota es PROPERTY_RULE sobre el certificado del SDP  |
...
### 1.4 Clase C — Simulación química / materiales (Reto 3)
...
- **Se implementa este mes (carril Reto 1, el nuestro):** QAOA p∈{1,2} con semillas INTERP +
  ...
  recocido simulado vía `neal` (idiom 5, 6 líneas), Kernighan–Lin (NetworkX) y muestreo uniforme.
...
- **Recetado, se implementa solo si el equipo toma el reto:** kernel cuántico + SVM/RF/SVM-RBF
  (Reto 2) y VQE/UCCSD + anclas HF/FCI/NumPy (Reto 3) — las recetas completas ya están (nota 02);
  el catálogo solo las posiciona.
- **Catalogado (stretch/ablación, sin compromiso):** warm-start QAOA, RQAOA, MA-QAOA, espectral,
  Goemans–Williamson, QNN, XGBoost, VQE hardware-efficient.
...
**Ninguna librería nueva entra al stack este mes:** todo lo que se implementa usa el stack ya
...
- **Solver SDP para Goemans–Williamson** (p. ej. cvxpy + SCS) — licencias esperadas Apache-2.0/MIT,
  **PENDIENTE verificación en vivo** antes de adoptar.
