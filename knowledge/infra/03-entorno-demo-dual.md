...
8. **Actualización S-F (2026-07-20) — ratificación verbal de Geovanni (19-jul) + auditoría de ratificación:**
   ...
   - **Modelos por API keys (supersede el punto 7.c):** nadie del equipo corre un modelo local con potencia útil ⇒ el `model_list` del mes son modelos por API (Anthropic + abiertos servidos por API), keys como secret files del despliegue; `ollama` queda como perfil opcional archivado. `replay` sigue siendo la config del día D — el air-gap se prueba igual (los fixtures se graban en los ensayos). Contrato completo: freeze §15.7 [S-F].
