...
| **Dramatiq**                                 | RabbitMQ / Redis                                                                      | ⚠️ no de primera clase (existe middleware asyncio — no verificado en vivo; chequeo solo si se reabre el candidato)   | **LGPL-3.0** ⚠️         | v2.2.0 (jun 2026), activo, 5.3k★                                 | Redis o RabbitMQ            |
...
7. **Chequeos declarados (al construir — no bloquean el freeze):** (a) confirmar `SKIP LOCKED` textual en el PLpgSQL de Procrastinate al integrar; (b) spike del defer transaccional compartiendo conexión con `EventStore`; (c) verificar en vivo el middleware asyncio de Dramatiq solo si alguien reabre ese candidato.
