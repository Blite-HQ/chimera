...
def resolver_cpsat(n, aristas):
    ...
    if recomputado != valor:  # paso 1 de trust/10 aplicado al corpus
        raise AssertionError(
            f"CP-SAT inconsistente: objetivo {valor} != recompute {recomputado}"
        )
    return {
...
def generar_instancia(instancia, convencion, n, ramas):
    ...
    if n <= FUERZA_BRUTA_MAX_N:
        ...
        notas.append(
            f"sin prueba de optimalidad: incumbente={cpsat.get('valor')} "
...
def main():
    ...
    for instancia, constructor in CASOS.items():
        ...
        margen = min(
            abs(abs(ESCALA_FLUJO * p - round(ESCALA_FLUJO * p)) - 0.5)
            for _, _, p in ramas
        )
        for convencion in CONVENCIONES:
            ...
            ruta.write_text(
                json.dumps(registro, sort_keys=True, indent=2, ensure_ascii=True)
                + "\n",
                encoding="utf-8",
            )
            resumen.append(
                (
                    ...
                    convencion,
                    n,
                    len(registro["aristas"]),
                    registro["optimo"],
                    "+".join(registro["metodos"]),
                    registro["solver_status"],
                )
            )
        print(
            f"{instancia}: {len(ramas)} ramas ({paralelas} paralelas agregadas), "
            f"margen minimo al umbral x.5 = {margen:.4f} (en unidades de 0.01 MW)"
        )
    print()
    print(
        f"{'instancia':10} {'convencion':10} {'n':>3} {'|E|':>4} {'optimo':>8} "
        f"{'metodos':18} status"
    )
    for fila in resumen:
        print(
            f"{fila[0]:10} {fila[1]:10} {fila[2]:>3} {fila[3]:>4} "
