...
| [`contract-freeze-anexo-canonicalizacion.md`](contract-freeze-anexo-canonicalizacion.md) | DRAFT (annex to the freeze): exact byte-level canonicalization for `provenance_hash`/`claim_digest`/`policy_digest` (RFC 8785 + DSSE rules) with test vectors.                                               |
