...
| [`studio/`](studio/)             | VIGENTE (2 docs)       | [`product-model.md`](studio/product-model.md) — Workspace⊃Project⊃Run doctrine, domain lenses (#78); authority for M1/M15/M17 · [`projects-workspaces.md`](studio/projects-workspaces.md) — F1.1: `project` is a persisted row, `workspace` is still a route segment only |
