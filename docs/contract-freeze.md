...
4. **Pendiente S-G (sin cambios — lista priorizada en la convergencia §10):** Studio a clase+AL +
   fixture/generador del certificado + `engine/verification/attestation.py` (aún en vocabulario
   viejo) · `scripts/verify-bundle.py` (checklist de 7 puntos) + api/compose reales · seed Pydantic
   de la Policy (materializa §6) · Registry/Pipeline reales · seeds de tests (falla sembrada,
   proyección `cancelled`, matriz interaction×profile, replay fail-closed).
