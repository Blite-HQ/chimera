...
class EntryPointRegistry:
    ...
    def list(self) -> tuple[CapabilityManifest, ...]:
        return tuple(
            self._capabilities[cap_id].manifest for cap_id in sorted(self._capabilities)
        )
...
def _append_load_failed(
    store: EventStore, *, entry_point: str, error_kind: str
) -> None:
    store.append(
...
def _select_version(candidates: list[Capability], pin: str | None) -> Capability | None:
