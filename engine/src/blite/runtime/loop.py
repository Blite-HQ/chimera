...
def execute_run(
    ...
    *,
    ...
    try:
        ...
        recorder.step_event("failed", resolve_step.model_copy(update={"status": "failed"}))
    ...
    try:
        ...
        recorder.append(
            ...
            {"job_id": job_id, "step_id": invoke_step.step_id, "error_kind": error_kind},
        )
        recorder.step_event("failed", invoke_step.model_copy(update={"status": "failed"}))
        recorder.fail_run(error_kind)
        return _projected_row()
    output_digest = recorder.digest_of(outputs)
    recorder.append(
        "capability.job.completed",
        {"job_id": job_id, "step_id": invoke_step.step_id, "output_digest": output_digest},
    )
    recorder.step_event(
        "completed",
