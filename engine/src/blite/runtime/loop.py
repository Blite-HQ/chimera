...
def _run_resolve_and_invoke(
    ...
    *,
    ...
    if resolve_step is None:
        return _TurnOutcome(output_digest=None, error_kind=MAX_STEPS_EXCEEDED, step_id=None)
    try:
        ...
        return _TurnOutcome(
            output_digest=None, error_kind=type(exc).__name__, step_id=resolve_step.step_id
        )
...
def _budget_after(
    ...
    *,
    ...
    exhausted = (
        budget.tokens is not None and would_spend_tokens > budget.tokens
    ) or (budget.cost_usd is not None and would_spend_cost > budget.cost_usd)
...
def _run_agentic_turn(  # noqa: PLR0913 — un turno completo (proponer→gobernar→ejecutar→journalizar→verificar) necesita los 5 componentes como parámetros
    ...
    *,
    ...
    spent_tokens, spent_cost_usd, budget_exhausted = _budget_after(
        budget, spent_tokens=spent_tokens, spent_cost_usd=spent_cost_usd, proposal=proposal
    )
    if budget_exhausted:
    ...
    _emit_plan_item_update(recorder, plan_item, plan_id=plan_id, run_id=run_id, status="ok")
