...
> **Working in a `git worktree`? Run `npx husky` once, first.** `core.hooksPath` points at
> `.husky/_`, which is generated by `prepare` and **not versioned** — a fresh worktree has
> the config but not the directory, so **git silently runs no hooks at all**. No error, no
> warning: your commits simply stop being checked. This is not hypothetical; it is how 21
> unformatted files and a secret-scanning gate that never ran got into `mejorado/base`.
>
> ```bash
> npx husky            # creates .husky/_ in this worktree
> git config core.hooksPath   # should print .husky/_ — and the directory must exist
> ```
